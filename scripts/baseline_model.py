...
def train_linear_regression(X_train, X_test, y_train, y_test, experiment_name="stock_prediction"):
...
def train_random_forest(X_train, X_test, y_train, y_test, experiment_name="stock_prediction"):
...
def train_lstm_model(X_train, X_test, y_train, y_test, experiment_name="stock_prediction"):
...
def main():
    ...
    try:
        ...
        lr_model, lr_run_id = train_linear_regression(X_train, X_test, y_train, y_test)
        
        # Train Random Forest model
        rf_model, rf_run_id = train_random_forest(X_train, X_test, y_train, y_test)
        ...
        X_scaled_df = pd.DataFrame(X_scaled, index=features.index, columns=features.columns)
        y_scaled_df = pd.DataFrame(y_scaled, index=target.index, columns=['close'])
        ...
        lookback = 5
        X_train_lstm, y_train_lstm = prepare_data_for_lstm(X_train_scaled, y_train_scaled, lookback)
        X_test_lstm, y_test_lstm = prepare_data_for_lstm(X_test_scaled, y_test_scaled, lookback)
        
        # Reshape for LSTM [samples, time steps, features]
        X_train_lstm = X_train_lstm.reshape((X_train_lstm.shape[0], lookback, X_train_scaled.shape[1]))
        X_test_lstm = X_test_lstm.reshape((X_test_lstm.shape[0], lookback, X_test_scaled.shape[1]))
        
        # Train LSTM model
        lstm_model, lstm_run_id = train_lstm_model(X_train_lstm, X_test_lstm, y_train_lstm, y_test_lstm)
        ...
        if best_model_type in ['linear_regression', 'random_forest']:
            ...
            print(f"Saved LSTM model to {model_path} and scalers to {scaler_X_path}, {scaler_y_path}")
        ...
        print(f"{model_name} model performed best with RMSE: {best_rmse:.4f}. Registering this model...")
