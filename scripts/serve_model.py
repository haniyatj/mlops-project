...
def load_saved_model(model_type):
    ...
    if model_type == "lstm":
        ...
        print(f"Scaler paths: {scaler_X_path}, {scaler_y_path}")  # Debug output
...
def serve_saved_model(model_type, data_path):
    ...
    try:
        ...
        features, actual = prepare_data_for_serving(data_path, target_col='close', lookback=5)
        ...
        if is_lstm:
            ...
            lookback = 5
            X_lstm, y_lstm = prepare_data_for_lstm(X_scaled_df, actual, lookback)
            
            # Reshape for LSTM [samples, time steps, features]
            X_lstm = X_lstm.reshape((X_lstm.shape[0], lookback, X_scaled_df.shape[1]))
...
def main():
    parser = argparse.ArgumentParser(description="Serve locally saved model for predictions")
    parser.add_argument("--model", default="linear_regression", 
                        help="Model type (linear_regression, random_forest, or lstm)")
    parser.add_argument("--data", default="data/AAPL_daily_cleaned.csv", help="Path to input data")
